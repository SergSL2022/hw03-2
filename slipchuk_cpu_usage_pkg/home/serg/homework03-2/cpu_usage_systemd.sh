...
while true; do
	...
	# Вибираємо відсоток завантаження CPU за допомогою top grep awk
	cpu_load=$(top -bn 1 | grep "Cpu(s)" | awk '{print $2}')
	# Записуємо результат в файл логу
	echo "$time_now CPU Load: $cpu_load%" >> /home/serg/homework03-2/cpu_usage.log
